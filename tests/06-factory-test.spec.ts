  ...
  describe('Failed cases', async () => {
    ...
    it("Shouldn't creating existing pool", async function () {
      ...
      await rejects(
        ...
        (err: Error) => {
          equal(err.message.includes('700'), true);
          return true;
        },
      );
      await factory.deployPool(
      ...
      await rejects(
        factory.deployPool(bob.pkh, 'fa2', alice.pkh, 'fa12', 1000, 1, 0, 0, 0),
        (err: Error) => {
          equal(err.message.includes('403'), true);
          return true;
        },
      ...
      await rejects(
        factory.deployPool(alice.pkh, 'fa12', bob.pkh, 'fa2', 1000, 1, 0, 0, 0),
        (err: Error) => {
          equal(err.message.includes('700'), true);
          return true;
        },
      ...
      await rejects(
        factory.deployPool(alice.pkh, 'fa2', bob.pkh, 'fa2', 1000, 1, 0, 2, 1),
        (err: Error) => {
          equal(err.message.includes('403'), true);
          return true;
        },
      ...
      await rejects(
        factory.deployPool(bob.pkh, 'fa2', alice.pkh, 'fa2', 1000, 1, 0, 1, 2),
        (err: Error) => {
          equal(err.message.includes('700'), true);
          return true;
        },

...
from constants import FAR_FUTURE
...
def form_set_position(lower_tick, upper_tick, liquidity):
    return dict(
        ...
        liquidity=liquidity,
        deadline=FAR_FUTURE,
        maximum_tokens_contributed={"x" : liquidity, "y" : liquidity},
        referral_code=0
    )

def form_update_position(pos_id, delta, recipient):
    return dict(
        position_id=pos_id,
        ...
        to_y=recipient,
        deadline=FAR_FUTURE,
        maximum_tokens_contributed={"x" : delta * delta, "y" : delta * delta},
        referral_code=0
    )

def form_swap(direction, amount_in, recipient):
    res = {}
    if direction == "x_to_y":
        res = dict(
            dx=amount_in,
        ...
        assert False, "Direction should bekw either x_to_y or y_to_x"

    return res


class LocalChain():
    def __init__(self, storage, default_view_results=None):
        ...
        self.default_view_results = default_view_results
    ...
    def execute(self, call, amount=0, sender=None, source=None, view_results=None):
        ...
        res = call.interpret(
            ...
            sender=sender,
            source=source,
            view_results=view_results or self.default_view_results,
            level=self.level
        )
        self.balance = new_balance
        self.storage = res.storage
    
        self.storage["positions"] = skip_nones(self.storage["positions"])
        self.storage["position_ids"] = skip_nones(self.storage["position_ids"])
        self.storage["ticks"] = skip_nones(self.storage["ticks"])
        self.storage["cumulatives_buffer"]["map"] = skip_nones(self.storage["cumulatives_buffer"]["map"])
        self.storage["token_metadata"] = skip_nones(self.storage["token_metadata"])
    ...
    def callback_view(self, call):
        res = call.callback_view(
            storage=self.storage,
        )
        return res
    ...
    def apply_transfers(self, transfers):
        for transfer in transfers:
            ...
            if "token_id" in transfer:
                assert transfer["token_id"] == 0, "No other token ids are supported yet"
